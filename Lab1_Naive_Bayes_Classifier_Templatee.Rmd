...
naiveBayes <- setRefClass("naiveBayes",
  methods = list(
    ...
    {
      spltmsg <- (strsplit(str_replace_all(message, "[[:punct:]]", ""), split = " "))[[1]]
      result_ham <- p_ham
      result_spam <- p_spam
      ...
      if(result_ham>0.001*result_spam){
        print("ham")
        return ("ham")
      }
      else{
    ...
    score = function(X_test)
    {
      ...
      for (x in X_test$Message){
        X_test[X_test$Message == x, ]  <- x[!x %in% splitted_stop_words]
      }
